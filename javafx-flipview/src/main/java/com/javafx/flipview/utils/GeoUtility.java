package com.javafx.flipview.utils;
...
public final class GeoUtility
{
                                             ...
                                             double fractionFrom0)
  {
    ...
    double[] p = GeoUtility.computePointOnLine (p0, p1, fractionFrom0);
  ...
  public static void extendLine (double[] p0, double[] p1, double toLength,
                                 double anchor)
  {
    double[] p = GeoUtility.computePointOnLine (p0, p1, anchor);
    
    double length0 = toLength * anchor;
    double length1 = toLength * (1.0 - anchor);
    
    GeoUtility.extendLine (p, p0, length0);
    GeoUtility.extendLine (p, p1, length1);
  }
  ...
  public static void extendLine (double[] p0, double[] p1, double toLength)
  {
    double oldLength = GeoUtility.length (p0, p1);
  ...
  {
    double[] v = GeoUtility.createVector (p0, p1);
    return length (v);
  }
  ...
  {
    return GeoUtility.length ((double) x0, (double) y0,
                            (double) x1, (double) y1);
  }
  ...
  public static double length (int[] x, int[] y, boolean isClosed)
  {
    ...
    for (int i = 0; i < nPoints-1; i++)
      length += GeoUtility.length (x[i], y[i], x[i+1], y[i+1]);

    // Add last leg if this is a polygon
    if (isClosed && nPoints > 1)
      length += GeoUtility.length (x[nPoints-1], y[nPoints-1], x[0], y[0]);
                                 ...
                                 int x, int y)
  {
    ...
    double length = GeoUtility.length (x0, y0, x1, y1);
    if (length == 0.0)
      return GeoUtility.length (x0, y0, x, y);
    ...
    length = GeoUtility.length (xp, yp, x, y);
    return length;
  }
   ...
   */
  public static double computeAngleInRadians (double[] p0, double[] p1, double[] p2)
  {
    double[] v0 = GeoUtility.createVector (p0, p1);
    double[] v1 = GeoUtility.createVector (p0, p2);

    double dotProduct = GeoUtility.computeDotProduct (v0, v1);

    double length1 = GeoUtility.length (v0);
    double length2 = GeoUtility.length (v1);
   ...
   * 
   * @param p0, p1, p2  Three points finding angle between [x,y,z].
   * @return            Angle (in degrees) between given points.
   */
  public static double computeAngleInDegrees (double[] p0, double[] p1, double[] p2)
  {
	  return computeAngleInRadians(p0, p1, p2) * 57.2957795;  // 1 radian = 57.2957795 degrees
  }
  ...
  {
    int s1 = GeoUtility.sameSide (x0, y0, x1, y1, x2, y2, x3, y3);
    int s2 = GeoUtility.sameSide (x2, y2, x3, y3, x0, y0, x1, y1);
                                                     ...
                                                     int x1, int y1)
  {
    ...
    if (GeoUtility.isPointInsideRectangle (x0, y0, x1, y1, lx0, ly0) ||
        GeoUtility.isPointInsideRectangle (x0, y0, x1, y1, lx1, ly1))
      return true;

    // If it intersects it goes through. Need to check three sides only.

    // Check against top rectangle line
    if (GeoUtility.isLineIntersectingLine (lx0, ly0, lx1, ly1,
                                         x0, y0, x1, y0))
      return true;
                                                         ...
                                                         int x1, int y1)
  {
    ...
    if (GeoUtility.isPointInsideRectangle (x[0], y[0], x0, y0, x1, y1))
      return true;
    ...
    for (int i = 1; i < x.length; i++) {
      if (x[i-1] != x[i] || y[i-1] != y[i])
        if (GeoUtility.isLineIntersectingRectangle (x[i-1], y[i-1],
                                                  x[i], y[i],
                                                  x0, y0, x1, y1))
          return true;
    }
                                                        ...
                                                        int x1, int y1)
  {
    ...
    if (n == 1)
      return GeoUtility.isPointInsideRectangle (x0, y0, x1, y1, x[0], y[0]);
    
    //
    // If the polyline constituting the polygon intersects the rectangle
    // the polygon does too.
    //
    if (GeoUtility.isPolylineIntersectingRectangle (x, y, x0, y0, x1, y1))
      return true;
  
    // Check last leg as well
    if (GeoUtility.isLineIntersectingRectangle (x[n-2], y[n-2], x[n-1], y[n-1],
                                              x0, y0, x1, y1))
      return true;
    ...
    // or separate.
    //
    if (GeoUtility.isPointInsidePolygon (x, y, x0, y0) ||
        GeoUtility.isPointInsideRectangle (x0, y0, x1, y1, x[0], y[0]))
      return true;
  ...
  public static double[] computePolygonCentroid (double[] x, double[] y)
  {
    ...
    double area = GeoUtility.computePolygonArea (x, y);
                                                 ...
                                                 double[] intersection)
  {
    ...
    double a0 = GeoUtility.equals (x0, x1, LIMIT) ?
                INFINITY : (y0 - y1) / (x0 - x1);
    double a1 = GeoUtility.equals (x2, x3, LIMIT) ?
    ...
    if (GeoUtility.equals (a0, a1)) {
      if (!GeoUtility.equals (b0, b1))
        return -1; // Parallell non-overlapping
      
      else {
        if (GeoUtility.equals (x0, x1)) {
          if (Math.min (y0, y1) < Math.max (y2, y3) ||
              Math.max (y0, y1) > Math.min (y2, y3)) {
            double twoMiddle = y0 + y1 + y2 + y3 -
                               GeoUtility.min (y0, y1, y2, y3) -
                               GeoUtility.max (y0, y1, y2, y3);
            y = (twoMiddle) / 2.0;
            x = (y - b0) / a0;
          }
          ...
          if (Math.min (x0, x1) < Math.max (x2, x3) ||
            ...
            double twoMiddle = x0 + x1 + x2 + x3 -
                               GeoUtility.min (x0, x1, x2, x3) -
                               GeoUtility.max (x0, x1, x2, x3);
            x = (twoMiddle) / 2.0;
            y = a0 * x + b0;
          }
          else return -1;
    ...
    if (GeoUtility.equals (a0, INFINITY)) {
      x = x0;
      y = a1 * x + b1;
    }
    else if (GeoUtility.equals (a1, INFINITY)) {
      x = x2;
      y = a0 * x + b0;
    }
    else {
    ...
    if (GeoUtility.equals (x0, x1)) {
      if (y0 < y1)
        distanceFrom1 = y < y0 ? GeoUtility.length (x, y, x0, y0) :
                        y > y1 ? GeoUtility.length (x, y, x1, y1) : 0.0;
      else
        distanceFrom1 = y < y1 ? GeoUtility.length (x, y, x1, y1) :
                        y > y0 ? GeoUtility.length (x, y, x0, y0) : 0.0;
    }
    else {
      if (x0 < x1)
        distanceFrom1 = x < x0 ? GeoUtility.length (x, y, x0, y0) :
                        x > x1 ? GeoUtility.length (x, y, x1, y1) : 0.0;
      else
        distanceFrom1 = x < x1 ? GeoUtility.length (x, y, x1, y1) :
                        x > x0 ? GeoUtility.length (x, y, x0, y0) : 0.0;
    }
    
    double distanceFrom2;
    if (GeoUtility.equals (x2, x3)) {
      if (y2 < y3)
        distanceFrom2 = y < y2 ? GeoUtility.length (x, y, x2, y2) :
                        y > y3 ? GeoUtility.length (x, y, x3, y3) : 0.0;
      else
        distanceFrom2 = y < y3 ? GeoUtility.length (x, y, x3, y3) :
                        y > y2 ? GeoUtility.length (x, y, x2, y2) : 0.0;
    }
    else {
      if (x2 < x3)
        distanceFrom2 = x < x2 ? GeoUtility.length (x, y, x2, y2) :
                        x > x3 ? GeoUtility.length (x, y, x3, y3) : 0.0;
      else
        distanceFrom2 = x < x3 ? GeoUtility.length (x, y, x3, y3) :
                        x > x2 ? GeoUtility.length (x, y, x2, y2) : 0.0;
    }
    
    return GeoUtility.equals (distanceFrom1, 0.0) &&
      GeoUtility.equals (distanceFrom2, 0.0) ? 1 : 0;
  }
                                                       ...
                                                       double x1, double y1)
  {
    ...
    for (int i = 0; i < nPoints; i++) {
      ...
      if (GeoUtility.equals (x2, x3) && GeoUtility.equals (y2, y3))
        continue;

      int type = GeoUtility.findLineSegmentIntersection (x0, y0, x1, y1,
                                                       x2, y2, x3, y3,
                                                       intersection);
      ...
      if (type == -2) { // Overlapping
        ...
        int side = GeoUtility.sameSide (x0, y0, x1, y1,
                                      x[p1], y[p1], x[p2], y[p2]);
  ...
  public static int[] createEllipse (int[] x, int[] y)
  {
    ...
    double axisAngle = GeoUtility.computeAngleInRadians (p0, p1, p2);
  
    // dx / dy  
    double dx = GeoUtility.length (x0, y0, x[1], y[1]);
    double dy = GeoUtility.length (x0, y0, x[0], y[0]) * Math.sin (axisAngle);
    ...
    // it to zero as well.
    if (!GeoUtility.equals (axisAngle, Math.PI/2.0, 0.1) && 
        !GeoUtility.equals (axisAngle, Math.PI,     0.1) &&
        !GeoUtility.equals (axisAngle, 0.0,         0.1)) {
                                   ...
                                   double length, double angle, double inset)
  {
    ...
    int[] pos1 = new int[2];
    GeoUtility.findPolygonPosition (x, y, length, pos1);
                                             ...
                                             int[] position)
  {
    ...
    for (int i = 1; i < x.length; i++) {
      double legLength = GeoUtility.length (x[i-1], y[i-1], x[i], y[i]);
